...
data <- read_yaml('../input/data.yaml')
charge_vars <- read_yaml('../input/charge-vars.yaml')
setup_vars <- read_yaml('../input/setup-vars.yaml')
@

\pagenumbering{arabic}
\section*{Key findings}

Inmate booking and release data from Pierce County jails shows that during the period from \Sexpr{data$start_date} through \Sexpr{data$end_date}, over \Sexpr{data$hold_count} inmates were the subject of detainer requests by Immigration and Customs Enforcement (ICE). Latinx inmates are heavily overrepresented in the population subjected to ICE detainers. Statistical analysis shows that, when controlling for other factors, inmates with an ICE detainer on their booking sheet were detained \num[round-precision=1]{\Sexpr{data$imm_hold_multiplier}} times longer than inmates without detainer requests, a disparity which raises significant human rights concerns.
...
Under Washington state's public records law, we requested a dataset of all individuals released from any Pierce County correctional facility between the dates of \Sexpr{data$start_date_full} and \Sexpr{data$end_date_full}. After dropping some records in the process of cleaning the data, as described below, this data set includes information regarding \Sexpr{data$unique_booking_ids} cases of individuals released during this period.\footnote{The individuals released during this period may have been booked during this period or prior; the earliest bookings included in the dataset occurred in \Sexpr{data$earliest_booking_year}. This analysis does not attempt to control for multiple repeat bookings of the same individual.}
...
In all, \Sexpr{data$hold_count} bookings (\num{\Sexpr{data$hold_percent}} percent of the total) in our sample were of individuals subject to ICE detainers.\footnote{In total, we identified \Sexpr{charge_vars$start_hold_count} bookings of individuals subject to ICE detainers; \Sexpr{charge_vars$start_hold_count - charge_vars$end_hold_count} of these cases were dropped in the data cleaning process based on the same criteria applied to other records.} Men were over-represented among people subject to ICE detainers: \num{\Sexpr{data$hold_male_percent}} percent of those with detainers, versus \num{\Sexpr{data$male_percent}} percent of other inmates, were male. Compared to inmates without detainers, individuals identified as ``Hispanic'' were also over-represented in this group: more than four-fifths (\num{\Sexpr{data$hispanic_hold_percent}} percent) of inmates with immigration holds were coded ``Hispanic'' by Pierce County, compared to only \num{\Sexpr{data$hispanic_percent}} percent of all inmates. Indeed, Latinx\footnote{We use the term ``Latinx'' to refer to those coded as ``Hispanic'' by Pierce County.} inmates had about a 1 in \Sexpr{data$hispanic_chance_of_hold} chance (\num{\Sexpr{data$hispanic_holds_as_percent_of_race}} percent) of being subject to a detainer if booked into a Pierce County correctional facility. Asians/Pacific Islanders were the next most likely group, at \num{\Sexpr{data$api_holds_as_percent_of_race}} percent (see Table \ref{tab:RaceHold}).

% table 1
<<race_hold_table,echo=FALSE,include=FALSE>>=

args <- list(inputfile='../input/race_hold_table.csv')

table <- read_csv(args$inputfile)

get_row <- function(rowid, fieldname) {
    paste0(
    	"\\makecell[l]{", as.character(table[rowid, 'race']), "} & ",
    	as.character(table[rowid, 'n']), " & ",
    	as.character(table[rowid, 'race_percent']), "\\%", " & ",
    	as.character(table[rowid, 'detainer_count']), " & ",
    	as.character(table[rowid, 'detainer_percent']), "\\%")}
@
...
In our sample, the maximum time spent in jail was \num{\Sexpr{data$max_jail_days}} days; the average time spent in jail was \num{\Sexpr{data$overall_mean_jail_days}} days, and the median was \num{\Sexpr{data$overall_median_jail_days}} days. \num{\Sexpr{data$felony_percent}} percent of bookings in our sample involved at least one felony booking charge, and \num{\Sexpr{data$misdemeanor_percent}} percent involved only misdemeanor booking charges.

The average jail stay for people subject to an ICE detainer was significantly longer than for those without detainers. People without detainers spent \num{\Sexpr{data$no_hold_mean_jail_days}} days in jail on average (with a median jail stay of \num{\Sexpr{data$no_hold_median_jail_days}} days), while people with detainers spent an average of \num{\Sexpr{data$hold_mean_jail_days}} days in jail (a median of \num{\Sexpr{data$hold_median_jail_days}} days). A t-test confirms that this difference is statistically significant at the $p < 0.001$ level.

% table 2, felony/misdemeanor figures
<<felony_misdemeanor, echo=FALSE,include=FALSE>>=

args <- list(inputfile='../input/felony_misdemeanor.csv')

table <- read_csv(args$inputfile)

get_row <- function(rowid, fieldname) {
    paste0(
    	as.character(table[rowid, 'felony_misdemeanor']), " & ",
    	as.character(table[rowid, 'No detainer']), "\\si{\\day}" , " & ",
    	as.character(table[rowid, 'Detainer']), "\\si{\\day}" , " & ",
    	as.character(table[rowid, '% increase']), "\\si{\\percent}" )}
@
...
<<regression_table,echo=FALSE,include=FALSE>>=

args <- list(inputfile='../input/regression_summary.csv')

table <- read_csv(args$inputfile)

get_row <- function(rowid, fieldname) {
	significance <- case_when(
		table[rowid, 'P>|t|'] < 0.001 ~ "***",
		table[rowid, 'P>|t|'] < 0.01 ~ "**,",
		table[rowid, 'P>|t|'] < 0.05 ~ "*",
		table[rowid, 'P>|t|'] >= 0.05 ~ "")
    paste0(
    	"\\makecell[l]{", as.character(table[rowid, 'index']), "} & ",
    	"\\num{", as.character(table[rowid, 'coef']), "} & ",
    	as.character(significance), " & ",
    	"\\num{", as.character(table[rowid, 'std err']), "} & ",
    	as.character(table[rowid, '95% CI']), " & ",
    	"\\num{", as.character(table[rowid, 'Impact']), "} \\si{\\percent}")
    	}

get_intercept <- function(rowid, fieldname) {
	significance <- case_when(
		table[rowid, 'P>|t|'] < 0.001 ~ "***",
		table[rowid, 'P>|t|'] < 0.01 ~ "**",
		table[rowid, 'P>|t|'] < 0.05 ~ "*",
...
\begin{ThreePartTable}
  ...
  \begin{center}
    ...
    \rowfont\bfseries
      ...
      ~~\Sexpr{get_row(13)} \\
      ~~\Sexpr{get_row(14)} \\
      \Sexpr{get_intercept(1)} \\
      \hline
    \end{tabu}
    \begin{tablenotes}
    N = \Sexpr{data$regression_n}. Adj. R-Squared = \num[round-precision=3]{\Sexpr{data$rsquared_adj}}. Reference categories: race = white; gender = female; offense = public order offenses. *$p <0.05$, **$p <0.01$, ***$p <0.001$ (two-tailed tests).
	\end{tablenotes}
  \end{center}
\end{ThreePartTable}
\end{table}

Overall, we found that detainered inmates experienced jail stays \num{\Sexpr{data$imm_hold_impact_percent}} percent (with 95 percent confidence intervals of \num{\Sexpr{(data$imm_hold_impact_lower * 100)}} percent to \num{\Sexpr{(data$imm_hold_impact_higher * 100)}} percent) longer than inmates without detainers, when controlling for other factors including race, number of charges, type, and seriousness of charge. This disparity is the most important of our findings, for it reveals a dramatically unequal encounter with the justice system for people facing immigration enforcement. The data indicates that this is disparity is not ocurring by chance, or as a result of the nature or number of charges against them. \emph{Therefore, we conclude that Pierce County processes individuals subjected to immigration detainers differently than those without.}
...
Our data set does offer some suggestions that bail may be a key factor. For example, our data shows that compared to the general population, very few detainered inmates are paying any bail. Among inmates without detainers, \Sexpr{data$no_hold_paid_bail} inmates, or \num{\Sexpr{(data$no_hold_paid_bail_percent * 100)}}  percent, paid bail for at least one charge; among detainered inmates, only \Sexpr{data$hold_paid_bail} inmates, or \num{\Sexpr{(data$hold_paid_bail_percent * 100)}} percent, did. A t-test confirms that this difference is statistically significant at the $p < 0.001$ level---the rate of payment of bail for inmates with an ICE detainer is less than the expected result if these variables were not related.
...
Inmates with detainers are also much less likely to have charges released on personal recognizance. Among inmates without detainers, \num{\Sexpr{data$no_hold_pr_percent}} percent of charges were released on personal recognizance, while for inmates with ICE detainers, only \num{\Sexpr{data$hold_pr_percent}} percent of charges were released on personal recognizance. This is also consistent with previous research findings such as that of Beckett and Evans (2015).
...
Aside from these concern for the rights of detainered inmates themselves, there are other reasons this matters. Their family members, including many U.S. citizen children, are deeply affected by their prolonged incarceration; the communities where they work and worship, the organizations in which they participate, all are affected by their absence. Lastly, the significant cost of their prolonged detention is not borne by the federal government, but by the taxpayers of Pierce County. During the period examined here, in fact, Pierce County spent more than \$\num[round-precision=2,round-mode=places,round-integer-to-decimal, ]{\Sexpr{data$total_extra_cost}} to hold the \Sexpr{data$hold_count} inmates with ICE detainers longer than they would have held them without detainers.\footnote{Pierce County's total cost per inmate is \$\num{\Sexpr{data$cost_per_day_cents / 100}} per day, according to a 2014 \href{<https://ofm.wa.gov/sites/default/files/public/legacy/reports/Correctional_Needs_and_Costs_Study2014.pdf>}{analysis published by the Washington State Office of Financial Management}. Based on our finding that inmates with detainers, on average, spent \num{\Sexpr{data$hold_mean_jail_days - data$no_hold_mean_jail_days}} more days in jail than inmates without detainers (see above, ICE Detainers and Length of Jail Stay), we estimate that the additional cost to Pierce County of holding an inmate with a detainer is approximately \$\num[round-precision=2,round-mode=places,round-integer-to-decimal]{\Sexpr{data$extra_cost_per_detainer_cents / 100}}. Counting only the \Sexpr{data$hold_count} inmates with ICE detainers in this study, the total cost ascends to \$\num[round-precision=2,round-mode=places,round-integer-to-decimal]{\Sexpr{data$total_extra_cost}} over what it would cost the county if they declined these voluntary detainer requests.} Pierce County has no legal obligation to cover these additional costs, and they yield no public safety benefit for County taxpayers.

\section*{Conclusions, recommendations, and future research}

This research documents that in Pierce County today, inmates with detainers experience justice in a dramatically different way than those without---\emph{not} because their cases involve tougher charges, but because of the specific way Pierce County processes ICE detainers. To restate the findings explained above: according to our regression model, in Pierce County today, a hypothetical Hispanic male charged with 2 counts, the most serious of which is a class C felony for a public order offense, would be expected to be held for about \Sexpr{as.character(data$predicted_time_detained_no_hold)} days. If subjected to a detainer, the same person would be expected to be held for \Sexpr{as.character(data$predicted_time_detained_hold)} days.
